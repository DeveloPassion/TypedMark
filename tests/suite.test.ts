...
test("records reproducible evidence for every golden vector", async () => {
  ...
  expect(evidence.summary).toEqual({ discovered: 79, executed: 77, passed: 77, failed: 0, skipped: 2, changed: 0 });
  expect(evidence.query_summary).toEqual({ executed: 18, passed: 18, failed: 0 });
...
test("checked-in evidence contains schema-valid reports and a passing summary", () => {
  ...
  expect(evidence.summary).toEqual({ discovered: 76, executed: 74, passed: 74, failed: 0, skipped: 2, changed: 0 });
  expect(evidence.query_summary).toEqual({ executed: 17, passed: 17, failed: 0 });
...
test("checked-in example exercise records bounded instantiation and offline validation", () => {
  ...
  const registry = new SchemaRegistry(schemaDirectory);
  for (const report of [exercise.source_report, exercise.instance_report, exercise.offline_report]) {
