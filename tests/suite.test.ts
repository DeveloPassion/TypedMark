...
test("records reproducible evidence for every golden vector", async () => {
  ...
  expect(evidence.summary).toEqual({ discovered: 71, executed: 69, passed: 69, failed: 0, skipped: 2, changed: 0 });
...
test("checked-in evidence contains schema-valid reports and a passing summary", () => {
  ...
  expect(evidence.summary).toEqual({ discovered: 69, executed: 67, passed: 67, failed: 0, skipped: 2, changed: 0 });
  expect(evidence.query_summary).toEqual({ executed: 16, passed: 16, failed: 0 });
