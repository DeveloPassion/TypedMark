...
test("records reproducible evidence for every golden vector", async () => {
  ...
  expect(evidence.summary).toEqual({ discovered: 61, executed: 59, passed: 59, failed: 0, skipped: 2, changed: 0 });
...
test("checked-in evidence contains schema-valid reports and a passing summary", () => {
  ...
  expect(evidence.summary).toEqual({ discovered: 59, executed: 57, passed: 57, failed: 0, skipped: 2, changed: 0 });
  expect(evidence.query_summary).toEqual({ executed: 13, passed: 13, failed: 0 });
