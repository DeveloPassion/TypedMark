...
export function materializeStarters(inputs: readonly StarterInput[], schemas: ReadonlyMap<string, TemplateSchema>, config: TemplateContext, runtime: MaterializationRuntime = {
...
}): Array<{ noteType: string; path: string; data: Record<string, unknown>; body: string }> {
  ...
  const result = inputs.map((input) => {
    ...
    const fill = (value: unknown, definition: FieldDefinition, explicit: boolean, present: boolean, field: string, set: (value: unknown) => void): void => {
      ...
      if (!present || placeholder) {
        ...
        else if (field === "title") value = basename(input.path, ".md");
      ...
      if (definition.type === "object" && isMapping(value)) {
        ...
        for (const name of new Set([...Object.keys(object), ...Object.keys(definition.fields ?? {})])) {
          ...
          if (!child) next[name] = structuredClone(object[name]);
          else fill(object[name], child, explicit && Object.hasOwn(object, name), Object.hasOwn(object, name), `${field}.${name}`, (filled) => { next[name] = filled; });
        }
        value = next;
      } else if (definition.type === "list" && definition.items && Array.isArray(value)) {
        const next: unknown[] = [];
        value.forEach((item, index) => fill(item, definition.items!, explicit, true, `${field}.${index}`, (filled) => { next[index] = filled; }));
        value = next;
    ...
    for (const field of new Set([...Object.keys(schema.frontmatter ?? {}), ...Object.keys(input.starter.data), ...Object.keys(supplied)])) {
      ...
      if (!definition) data[field] = structuredClone(source[field]);
      else fill(source[field], definition, explicit, Object.hasOwn(source, field), field, (value) => { data[field] = value; });
    }
    data.note_type = input.noteType;
    ...
    if (Array.isArray(data.tags)) {
      ...
      for (const tag of tags) if (!seen.has(tag.normalize("NFC"))) { data.tags.push(tag); seen.add(tag.normalize("NFC")); }
    }
    return { noteType: input.noteType, path: input.path, data, body: input.starter.body };
  });
